#!/usr/bin/env bash
# Launcher for the RobCraft editor, used by the installed .desktop entry.
...
# Installed layout: <prefix>/lib/robcraft/launch_robcraft_editor.sh
...
if [[ -f "${LOCAL_SETUP}" ]]; then
    ...
    BIN="${PREFIX}/lib/robcraft/robcraft-editor"
else
    # Source checkout layout: <repo>/robcraft/scripts/launch_robcraft_editor.sh
    WORKSPACE_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
    source "${WORKSPACE_ROOT}/install/setup.bash"
    cd "${WORKSPACE_ROOT}"
    BIN="${WORKSPACE_ROOT}/install/robcraft/lib/robcraft/robcraft-editor"
fi
set -u
